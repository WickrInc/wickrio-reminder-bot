...
class Reminder {
    ...
    _parse(text) {
        // TODO: use forwardDate: true here, but it breaks the .text value
        // e.g. instead of "in four hours" it becomes "four hours"
        let dateParse = chrono.parse(text)[0];
    ...
    _findAction(text, dateParse) {
        ...
        // parse out "me" and "to" from beginning of action
        // if "to" exists, the reminder is in the infinitive form
        let match = actionText.match(/(?:me\s)?(to\s)?(.*)/);
        if (match == null) {
